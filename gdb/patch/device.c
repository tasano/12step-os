...
void sim_device_init(SIM_DESC sd, unsigned char *memory);
int sim_device_sleep(SIM_DESC sd, unsigned char *memory);
int sim_device_run(SIM_DESC sd, unsigned char *memory);
...
static int search_freepty(SIM_DESC sd)
{
  int fd;
#ifdef __FreeBSD__
...
#endif
  sim_io_printf (sd, "Free pty not found.\n");
  return -1;
}

static void serial_init(SIM_DESC sd, unsigned char *memory)
{
  ...
  serial_fd = search_freepty(sd);
}

static int is_send_enable(SIM_DESC sd, int fd)
{
  fd_set fds;
  struct timeval tm;
  int ret;
...
static int is_recv_enable(SIM_DESC sd, int fd)
{
  fd_set fds;
  struct timeval tm;
  int ret;
...
static void send_byte(SIM_DESC sd, int fd, unsigned char c)
{
#ifdef USE_CALLBACK
  host_callback *sim_callback = STATE_CALLBACK (sd);
...
static unsigned char recv_byte(SIM_DESC sd, int fd)
{
  unsigned char c;
  int size;
#ifdef USE_CALLBACK
...
static int serial_run(SIM_DESC sd, unsigned char *memory)
{
  volatile struct h8_3069f_sci *sci = DEFAULT_SCI;
  unsigned char c;
  ...
  if (sci->scr & H8_3069F_SCI_SCR_TE) {
    if (!(sci->ssr & H8_3069F_SCI_SSR_TDRE)) {
      if (is_send_enable(sd, serial_fd)) {
	send_byte(sd, serial_fd, sci->tdr);
  ...
  if (sci->scr & H8_3069F_SCI_SCR_RE) {
    if (!(sci->ssr & H8_3069F_SCI_SSR_RDRF)) {
      if (is_recv_enable(sd, serial_fd)) {
	c = recv_byte(sd, serial_fd);
	sci->rdr = c;
...
static int timer16_init(SIM_DESC sd, unsigned char *memory)
{
...
static int timer16_run(SIM_DESC sd, unsigned char *memory)
{
...
static void ether_init(SIM_DESC sd, unsigned char *memory)
{
  char ifname[16];
  ...
  if (ether_fd >= 0) {
    ...
    if (ioctl(ether_fd, TUNSETIFF, (void *)&ifr) < 0) {
      sim_io_printf (sd, "Fail to ioctl().\n");
      close(ether_fd);
      ether_fd = -1;
    }
...
#endif
  if (ether_fd < 0) {
    sim_io_printf (sd, "Free tap not found.\n");
    sim_io_printf (sd, "Need executing kldload if_tap.\n");
    return;
  }
...
static int ether_run(SIM_DESC sd, unsigned char *memory)
{
  int i, c, ret;
  ...
  if (NE2000_CR & NE2000_CR_TXP) {
    ...
    if (ether_fd >= 0)
      ignored = write(ether_fd, buf, size);
...
void sim_device_init(SIM_DESC sd, unsigned char *memory)
{
  ...
  serial_init(sd, memory);
  timer16_init(sd, memory);
  ether_init(sd, memory);
...
int sim_device_sleep(SIM_DESC sd, unsigned char *memory)
{
  ...
  if (fd < 0) {
    sim_io_printf (sd, "not found opened device.\n");
  }
...
int sim_device_run(SIM_DESC sd, unsigned char *memory)
{
  ...
  vec    = timer16_run(sd, memory);
  vector = (vec < vector) ? vec : vector;

  vec    = serial_run(sd, memory);
  vector = (vec < vector) ? vec : vector;

  vec    = ether_run(sd, memory);
  vector = (vec < vector) ? vec : vector;
