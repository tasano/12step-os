...
typedef struct _kz_msgbox {
     ...
     * よって、構造体のサイズが2の累乗になっていないと、構造体の配列のインデックス計算で
     * 乗算が使われて「___mulsi3が無い」などのリンク・エラーになる場合がある。
     * (2の累乗ならばシフト演算が利用できるので問題は出ない)
     * 対策として、サイズが2の累乗になるようにダミー・メンバで調整する。
...
static kz_thread_id_t thread_recv( kz_msgbox_id_t id, int* sizep, char** pp )
{
    ...
    if ( mboxp->head == NULL ) {
        /*
         * メッセージ・ボックスにメッセージが無いので、
         * スレッドをスリープさせる。(システム・コールがブロックする)
         */
        return -1;
    }
...
static void call_functions( kz_syscall_type_t type, kz_syscall_param_t* p )
{
    ...
    case KZ_SYSCALL_TYPE_SEND: /* kz_send() */
        p->un.send.ret =
            thread_send( p->un.send.id, p->un.send.size, p->un.send.p );
        break;
...
static void srvcall_proc( kz_syscall_type_t type, kz_syscall_param_t* p )
{
     ...
     * 参照している部分があり(例えば thread_send() など)、
